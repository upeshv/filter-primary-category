...
function addPrimaryCategory(categoryId, categoryLabel) {
  jQuery('#tenup_fpc_primary_category').append(jQuery('<option>', {
      value: categoryId,
      text: categoryLabel,
  }));
}
...
function removePrimaryCategory(categoryId) {
  jQuery('#tenup_fpc_primary_category option[value="' + categoryId + '"]').remove();
}
...
jQuery(document).ready(function() {
   ...
   */
  var getPrimaryCategory = jQuery('#tenup_fpc_primary_category').data('primary-category');

  jQuery('#tenup_fpc_primary_category').val(getPrimaryCategory);
